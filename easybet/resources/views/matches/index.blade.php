                        ...
                        <div class="card-body">
                            <form action="">
                                ...
                                <select name="teams_id"
                                        class="browser-default custom-select form-control @error('teams_id') is-invalid @enderror mb-2">
                                ...
                                <select name="categories_id"
                                        class="browser-default custom-select form-control @error('teams2_id') is-invalid @enderror mb-2">
                ...
                <div class="col-12">
                    <div class="card bg-dark text-white">
                        ...
                        <div class="card-body">
                            <ul class="list-group">
                                @foreach($categories as $category)
                                    <a href="{{route('matches.categories.game', $category->id)}}">
                        ...
                        <ul class="nav nav-pills bg-dark rounded justify-content-center p-4">
                            @foreach($games as $game)
                                <li class="nav-item p-2 mr-5">
                                    <a href="{{route('matches.game', $game->id)}}"><img
                                            src="{{asset('storage/'.$game->image)}}" alt="" class="img-thumbnail p-0"
                                            width="60"></a>
                                </li>
                            @endforeach
                        </ul>
                        ...
                        <tr>
                            ...
                            @if(Auth::user()->admin == 1)
                                ...
                                @else
                                    <td><a class="btn btn-success"
                                           href="{{ route('matches.edit', $match->id) }}">Edit</a></td>
                                @endif
                                <form
                                    action="{{ route($match->deleted_at? 'matches.force.destroy' : 'matches.destroy', $match->id) }}"
                                    method="post">
                                    @csrf
                                    @method('DELETE')
                                    <td colspan="2">
                            ...
                            @else
                                <td><a class="btn btn-primary"
                                       href="{{ route('matches.show', $match->id) }}">Details</a></td>
                            @endif
